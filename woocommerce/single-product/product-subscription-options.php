...
<script>
	jQuery(function($) {
        if(jQuery('body').hasClass('woofunnels-pop')) {
            console.log('got here!');
            jQuery('.choose-label .subscribe-prompt').text(jQuery('.choose-label .subscribe-prompt').text().replace('Subscribe & Save 15%', 'Subscribe & Save 20%'));
           	jQuery('body').removeClass('woofunnels-pop');
        }
        });
        </script>
...
<div class='qty-container'><label>Select a Quantity</label><select id='selectQty' class='input-text qty'><option value='1'>1</option><option value='2'>2</option><option value='3'>3</option><option value='4'>4</option><option value='5'>5</option><option value='6'>6</option><option value='7'>7</option><option value='8'>8</option><option value='9'>9</option></select></div><label>I would like to</label><ul class='purchase-options'>\n".
	...
	($subscription_options ? "<li". (isset($selected_subscription_option) ? " class='selected'" : "") .">".
		...
		"<label for='subscriptions-list' class='subscription-container'><span class='choose-label'><span class='subscribe-prompt'>". ($prompt ? strip_tags($prompt) : __('Subscribe & Save 15%')) . "</span><ul class='subscription-price'>". implode('', $hidden_options) ."</ul></label>\n".
