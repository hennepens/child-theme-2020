  ...
  add_action( 'wp_enqueue_scripts', function() {
    ...
    if( jQuery( ".variations_form select" ).length  ){
     ...
     if(jQuery("body").hasClass("clicked-subscription")) {
      ...
      jQuery(".subscription-price").find(\'&mdash; save\').each(function(){
        jQuery(this).text(jQuery(this).text().replace(\'&mdash; save\',\' \'));
      });
...
function bbloomer_wholesale_portal_content() {
   echo '<h3>Wholesale Portal</h3><p>Welcome to the wholesale portal. As a wholesale customer, you can select any number of cases from our product line and receiving keystone pricing when you order at least 1 case.</p>';
  ...
  if ( $variation_certs ) {
    ...
    if ( $current_certs && ! is_wp_error( $current_certs ) ) {
      ...
      foreach ($current_certs as $cert) {
        ...
        if ($cert_name) {
          echo '<span class="cert-tag ' . $cert_name . '" data-toggle="tooltip" data-html="true" data-placement="bottom" title="'. $cert_description .'">' . $cert_name . '</span>';
        }
      }
...
function wpse_new_mail_from( $old ) {
    return 'support@hennepens.com'; // Edit it with your email address
}
