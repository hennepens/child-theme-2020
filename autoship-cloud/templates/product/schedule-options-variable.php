...
<script>
  jQuery(document).ready(function(){
    jQuery("input[type='radio']").click(function(){
      jQuery(".autoship-type").removeClass('active');
      jQuery(this).closest(".autoship-type").toggleClass('active');
...
<div id="autoship-schedule-options-variable" class="autoship-schedule-options-variable autoship-schedule-options hidden <?php echo $skin['container']?>" data-autoship-variation-id="0" data-autoship-id="<?php echo $product->get_id(); ?>" data-autoship="<?php echo $autoship_attr;?>">
    ...
    <div class="autoship-type autoship-yes <?php echo $skin['type']?>">
      ...
      <label class="autoship-label <?php echo $skin['label']?>">
        ...
        echo autoship_checkout_recurring_variable_discount_string2( $product->get_id() );
    ...
    <div class="autoship-frequency <?php echo $skin['frequency']?> flex-column">
      <h6 class="prompt-heading text-right">Choose a Subscription Schedule</h6>
      <div class="d-flex justify-content-end align-items-center">
        <label class="<?php echo $skin['label']?>"><?php echo apply_filters('autoship_frequency_label', __( 'Deliver', 'autoship' ), $product ); ?></label>
